...
namespace Pace\RestReport;

use Pace\RestModel;
use InvalidArgumentException;
use Pace\Enum\ReportExportType;
use Pace\RestServices\ReportService;
...
class Builder
{
     ...
     * @param RestModel $report
     */
    public function __construct(ReportService $service, RestModel $report)
    {
    ...
    {
        $this->baseObjectKey = $key instanceof RestModel ? $key->key() : $key;
     ...
     *
     * @return \Pace\Report\File
     */
    public function get(): \Pace\Report\File
    {
        $report = $this->service->executeReport($this->toWrapper());

        return \Pace\Report\File::fromBase64($report['content'], $this->mediaTypes[$this->report->exportType] ?? null);
    }
